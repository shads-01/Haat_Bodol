...
const ChatPage = () => {
  ...
  const loadMessages = async (otherUserId) => {
    try {
      ...
      // Find the user info for the selected conversation
      const convo = conversations.find(c => c._id === otherUserId);
      if (convo) {
  ...
  const startNewConversationWithUser = async (userId) => {
    try {
      ...
      const existingConvo = conversations.find(c => c._id === userId);
              ...
              <List>
                {conversations.map(convo => (
                  <ConversationListItem
                    key={convo._id}
                    button
                    selected={selectedConversation === convo._id}
                    onClick={() => loadMessages(convo._id)}
                  >
                    <ListItemAvatar>
                      <Badge
